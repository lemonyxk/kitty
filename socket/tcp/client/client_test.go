...
func initServer(fn func()) {
	...
	// create server
	webSocketServer = &server.Server{Host: "127.0.0.1:8667"}
	...
	webSocketServerRouter.Group("/hello").Handler(func(handler *server.RouteHandler) {
		handler.Route("/world").Handler(func(conn *server.Conn, stream *socket.Stream) error {
			return conn.JsonEmit(socket.JsonPackage{
				Event: "/hello/world",
				Data:  "i am server",
			})
		})
...
func initClient(fn func()) {
	// create client
	client = &Client{Host: "127.0.0.1:8667", Reconnect: true, AutoHeartBeat: true}
