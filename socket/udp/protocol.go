...
var OpenMessage = []byte{0x0, 0x0, 0x3, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0}
var CloseMessage = []byte{0x0, 0x0, 0x4, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0}
...
func isHeaderInvalid(message []byte) bool {
	...
	// message type
	if message[2] != socket.BinData &&
		message[2] != socket.PingData &&
		message[2] != socket.PongData &&
		message[2] != socket.OpenData &&
		message[2] != socket.CloseData {
		return false
	}
